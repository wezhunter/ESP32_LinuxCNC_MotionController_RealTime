...
static esp_err_t enc28j60_verify_id(emac_enc28j60_t *emac)
{
  ...
  MAC_CHECK(emac->revision >= ENC28J60_REV_B1
            && emac->revision <= ENC28J60_REV_B7, "wrong chip ID", out, ESP_ERR_INVALID_VERSION);
out:
  return ret;
}
...
static esp_err_t enc28j60_setup_default(emac_enc28j60_t *emac)
{
  ...
  MAC_CHECK(enc28j60_register_write(emac, ENC28J60_MACON3, MACON3_PADCFG0 | MACON3_TXCRCEN | MACON3_FRMLNEN) == ESP_OK,
...
static esp_err_t emac_enc28j60_receive(esp_eth_mac_t *mac, uint8_t *buf, uint32_t *length)
{
  ...
  // read packet content
  MAC_CHECK(enc28j60_read_packet(emac, enc28j60_rx_packet_start(emac->next_packet_ptr, ENC28J60_RSV_SIZE), buf,
                                 rx_len) == ESP_OK,
